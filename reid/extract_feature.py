...
def get_dataloader(batch_size=64, workers=4):
    ...
    gallery_loader = DataLoader(TestDataset(root=gallery_dir, transform=test_transformer), 
                                        ...
                                        num_workers=workers, 
                                        shuffle=False, pin_memory=True)
    return single_query_loader, multi_query_loader, gallery_loader
...
def evaluate():
    ...
    single_query_loader, multi_query_loader, gallery_loader = get_dataloader()

    print ('Create and load pre-trained model...')
    model = models.create('resnet50',dropout=0.0, num_features=2048, num_classes=751)
    checkpoint = load_checkpoint('./logs/deep-person-1-new-augmentation/market1501-resnet50/model_best.pth.tar')
    model.load_state_dict(checkpoint['state_dict'])
    model = nn.DataParallel(model).cuda()
    model.eval()

    print ('Extract single_query&gallery feature...')
